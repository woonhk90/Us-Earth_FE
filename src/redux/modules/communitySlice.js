...
/* ----------------------------- 전체 그룹 모임 정보 출력 ----------------------------- */
export const __getCommunity = createAsyncThunk(
  "usearth/__getCommunity",
  async (payload, thunkAPI) => {
    try {
      ...
      const data = await instance.get(
        `${API_URL}/community?page=${payload.page}&size=10`
      );
...
/* -------------------------------- 커뮤니티 상세보기 ------------------------------- */
export const __getCommunityDetail = createAsyncThunk(
  "usearth/__getCommunityDetail",
  async (payload, thunkAPI) => {
    try {
...
export const __updateCommunityJoin = createAsyncThunk(
  "usearth/__updateCommunityJoin",
  async (payload, thunkAPI) => {
    try {
      const data = await tokenInstance.patch(
        `/join/${payload.communityId}`,
        payload
      );
...
/* ------------------------------ 인증 게시글 목록 출력 ------------------------------ */
export const __getCommunityCertify = createAsyncThunk(
  "usearth/__getCommunityCertify",
  async (payload, thunkAPI) => {
    try {
      if (payload.page === "0" || payload.page === 0) {
      ...
      const data = await instance.get(
        `/community/${payload.communityId}/proof?page=${payload.page}&size=3`
      );
...
/* -------------------------------- 활발 그룹 출력 -------------------------------- */
export const __getPopularGroupItemList = createAsyncThunk(
  "usearth/__getPopularGroupItemList",
  async (payload, thunkAPI) => {
    try {
...
/* ------------------------------- 마감임박 그룹 출력 ------------------------------- */
export const __getNewGroupItemList = createAsyncThunk(
  "usearth/__getNewGroupItemList",
  async (payload, thunkAPI) => {
    try {
...
export const communitySlice = createSlice({
  ...
  reducers: {
    ...
    hasMoreFn: (state, action) => {
      state.hasMore = action.payload;
    },
    certifyHasMoreFn: (state, action) => {
      state.certifyHasMore = action.payload;
    },
    ingVal: (state, action) => {
      /* state.community = []  */
    },
    ...
    pagePlus: (state) => {
      state.pageReset = state.pageReset + 1;
    },
    searchPlus: (state, action) => {
      state.searchPlus = action.payload;
    },
  },
  extraReducers: {
